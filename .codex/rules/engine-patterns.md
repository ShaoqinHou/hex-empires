...
- **`Math.random()` anywhere in `packages/engine/`** -> BLOCK. The optional check utility (`check-edited-file.sh`) already greps for this.
...
- **When Reviewer finds a novel engine-side trap**, log a `[PRINCIPLE-GAP]` entry in `issues.md` with the pattern; periodically a human promotes repeat gaps into `emerging-traps.md`, then into the relevant `.codex/rules/` document once confirmed.
