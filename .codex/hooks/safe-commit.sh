#!/bin/bash
# safe-commit.sh — guard against worktree-isolation leaks.
#
# Phase 6c exposed a worktree leak: a blind-eval agent was assigned to a
...
#
# This script is an opt-in machine guard. When a worktree is spawned for
# agent work, the orchestrator writes a sentinel file at the worktree
# root (`.codex/worktree-sentinel`) whose contents are the worktree's
...
# with a message explaining what went wrong. The caller then has to investigate.

set -uo pipefail

# Bail fast if there's no sentinel — i.e. this is normal-mode work, not
# a guarded worktree session.
SENTINEL_FILE=".codex/worktree-sentinel"
if [ ! -f "$SENTINEL_FILE" ]; then
  exit 0
fi

# Tool wrappers can deliver the attempted shell command as JSON on stdin.
# Manual callers may pipe the same shape.
