...
1. **Art / audio fidelity** — we use hand-SVG + Game-icons.net + procedural sound. We are cloning the RULES, not the audiovisual presentation. This was locked in the original project decisions ("art budget", "sound budget").
...
The existing gap matrix (`.codex/gdd/gap-matrix.md`) was written before this commitment. Under "clone" semantics, its priority column should be re-read as follows:
...
- `.codex/gdd/README.md` — the full GDD taxonomy + source list
- `.codex/gdd/gap-matrix.md` — the one-page convergence plan
- Legacy project notes — original art/sound/game-feel scope commitments
