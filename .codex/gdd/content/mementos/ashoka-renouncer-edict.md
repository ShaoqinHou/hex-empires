# Pillar of Ashoka - Civ VII

**Slug:** `ashoka-renouncer-edict`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
Ashoka World Renouncer Leader Path memento. Buddhist edicts on stone pillars.

## Stats / numeric attributes

- **Path:** Leader — Ashoka World Renouncer
- **Unlock level:** Leader Path L2
- **Effect:** Faith bonus per city
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Faith bonus per city `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    level: 2
    description: "Faith bonus per city"
```
