...
Jose Rizal Leader Path memento. Polymath artistic tools.

## Stats / numeric attributes

- **Path:** Leader — Jose Rizal
- **Unlock level:** Leader Path L9
- **Effect:** Culture from Great Works
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Culture from Great Works `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: jose-rizal
    level: 9
    description: "Culture from Great Works"
```
