...
- `SettlementState.specialization` / `SettlementState.lockedTownSpecialization` — (RW) — Town Focus at pop 7 (null for Cities)
...
- **On SET_SPECIALIZATION** — lock the first non-Growing Town Focus for the age
...
- **On UPGRADE_SETTLEMENT** — gain production queue; specialist slots available
...
**Status tally:** 5 MATCH / 3 CLOSE / 1 DIVERGED / 0 MISSING / 0 EXTRA
**Audit:** [.codex/gdd/audits/population-specialists.md](../audits/population-specialists.md)
**Highest-severity finding:** F-03 — Specialist adjacency amplification exists but depends on spatial specialist data (CLOSE, HIGH)
**Convergence status:** Divergent — 1 finding(s) require(s) architectural refactor

_(Full details in audit file. 9 total finding(s). Regenerated by `.codex/scripts/aggregate-audits.py`.)_
