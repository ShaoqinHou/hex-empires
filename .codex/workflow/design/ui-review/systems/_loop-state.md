...
- **Subagent `Write` state machine** — see `.codex/rules/loop-and-continuous-mode.md` § gotcha #5, 4 states. State D (post-push) = Write works. If regressed: Bash heredoc fallback.
- **Commit-per-sub-step is enforced** — implementer brief counts commits, flags mismatches. Updated in `implementer.md` 2026-04-18.
- **Hook/phase collision**: `commit-review.sh` now checks `.codex/workflow/scratch/.phase-in-flight` and skips review if present. Parent creates the lock before spawning phase-impl, removes after cherry-pick.
...
- **Agent teams are NOT production-ready** (3 undocumented corners). Use bounded Codex subagents for multi-perspective reads.
...
Each phase's completion notification triggers the next. No timed loop. Subagents use the Codex `explorer` or `worker` roles. Commit per sub-step. Verify the assigned model class on every return.
...
| audit-3 | Designer -> strong model + gotcha #5 rewrite + teams gotcha + model-routing rule | **DONE** | `bd65ba5` |
...
3. Check `.codex/workflow/scratch/.phase-in-flight` — if it exists, a phase was in-flight when the session ended. Either resume it or remove the lock manually.
...
5. Spawn next phase's subagent:
   ...
   - Spawn a Codex `worker` with the implementer brief. `implementer.md` is a role template with enforcement rules and design handoff patterns.
...
- Parent should use a strong model for integration and review. If the assigned model is weaker than the task requires, note it and escalate.
