#!/bin/bash
# KNOWN-LIMITED ON PRIOR AGENT RUNTIME (2026-04-18)
# The task `output_file` path was a 0-byte placeholder; the subagent JSONL
# transcript was not streamed to disk during the run. Mid-run hang detection
# via size-sampling is therefore disabled unless the current runtime exposes a
# progressively written output file.
#
...
#   bash .codex/workflow/scripts/sample-agent-output.sh \
...
# Review with: bash .codex/workflow/scripts/agent-timing-report.sh <agent-id>
...
# Windows path conversion: some runtimes surface paths like C:\Users\... on
# Windows. MINGW Bash needs /c/Users/... for stat to resolve the file. Convert
# transparently.
case "$FILE" in
  [A-Za-z]:\\*|[A-Za-z]:/*)
...
LOG_DIR=".codex/workflow/scratch"
