...
Panels are the explicit, player-triggered surfaces. The HUD layer covers the rest — hover tooltips, toasts, validation feedback, turn transitions, minimap, hint badges. It follows the same central-manager + shared-shell philosophy as panels, with its own rules tuned for transient, cursor-driven UI.

The foundation has shipped (HUD cycles a–k, M-HUD1 → M-HUD3): `HUDManager`, `TooltipShell`, `hudRegistry`, and `hud-tokens.css` all live in `packages/web/src/ui/hud/` and `packages/web/src/styles/`. The full rule set is `.claude/rules/ui-overlays.md`, the step-by-step guide is `.claude/skills/add-hud-element/`, and the running registry of every HUD element currently in the tree is `.claude/workflow/design/hud-elements.md`.

- **Location:** HUD foundation (`HUDManager`, `TooltipShell`, `hudRegistry`, `TooltipOverlay`, `UrbanPlacementHintBadge`) lives in `packages/web/src/ui/hud/`. Toast / fixed-corner overlays (`Notifications`, `ValidationFeedback`, `CombatHoverPreview`, `EnemyActivitySummary`, `TurnTransition`, `Minimap`) live in `packages/web/src/ui/components/` and consume the shell or its sibling fixed-position helpers.
- **Central manager:** `HUDManager` owns per-overlay registration, stack-cycle indices for stacked entities, and coordinated dismiss (ESC) — mirroring `PanelManager` for panels.
...
- **Styling:** tokens only — `var(--panel-*)` for shared chrome values and HUD-specific tokens from `hud-tokens.css`. Never raw hex, never Tailwind color utilities for overlay chrome.
