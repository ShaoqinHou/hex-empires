...
**Status tally:** 0 MATCH / 0CLOSE / 3 DIVERGED / 2 MISSING / 1 EXTRA
**Audit:** [.claude/gdd/audits/crises.md](../audits/crises.md)
**Highest-severity finding:** F-01 — Crisis is a one-shot narrative event, not persistent age-phase system (DIVERGED, HIGH)
**Convergence status:** Divergent — 3 finding(s) require(s) architectural refactor

_(Full details in audit file. 6 total finding(s). Regenerated by `.claude/scripts/aggregate-audits.py`.)_
