...
**Status tally:** 0 MATCH / 4CLOSE / 3 DIVERGED / 4 MISSING / 1 EXTRA
**Audit:** [.claude/gdd/audits/map-terrain.md](../audits/map-terrain.md)
**Highest-severity finding:** F-01 — Terrain model uses Civ VI per-tile types, not VII biome+modifier (DIVERGED, HIGH)
**Convergence status:** Divergent — 3 finding(s) require(s) architectural refactor

_(Full details in audit file. 12 total finding(s). Regenerated by `.claude/scripts/aggregate-audits.py`.)_
