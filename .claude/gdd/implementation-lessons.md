...
Grep-based `.claude/scripts/verify-retirements.sh` found **8 of 26 retirement
...
`.claude/scripts/verify-retirements.sh` is a fast shell script that enumerates
...
- `.claude/scripts/verify-retirements.sh` — grep-based checker
- `packages/engine/src/__tests__/retirement-invariants.test.ts` — test-based
- `.claude/gdd/implementation-tracker.md` — workpack history
- `.claude/gdd/audits/*.md` — audit findings with retirement recommendations
