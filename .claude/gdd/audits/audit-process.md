...
**Input:**
- `.claude/gdd/systems/<slug>.md` — the mechanics doc (what VII does)
- `.claude/gdd/content/<category>/` — relevant fact cards (values, rules)
...
**Output:**
- `.claude/gdd/audits/<slug>.md` — the audit report (from `_template-audit.md`)
- Updated `Mapping to hex-empires` section in `.claude/gdd/systems/<slug>.md`
- New rows appended to `.claude/gdd/convergence-tracker.md`
...
Output: .claude/gdd/audits/<slug>.md
Template: .claude/gdd/audits/_template-audit.md
...
After each audit, append its findings to `.claude/gdd/convergence-tracker.md`. One row per finding. Columns:
...
After the audit, edit `.claude/gdd/systems/<slug>.md` and replace the empty `Mapping to hex-empires` section with:
