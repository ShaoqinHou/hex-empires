...
**Purpose.** A preamble to include in every subagent brief where the agent is expected to produce files on disk. Designed to survive the `Write`/`Bash` permission-cache bug documented in `.claude/rules/loop-and-continuous-mode.md` gotcha #5 without polluting parent context with inline content dumps.
...
Output: `.claude/gdd/content/<category>/<slug>.md`
...
- `.claude/rules/loop-and-continuous-mode.md` § gotcha #5 (Write permission state machine)
- `.claude/rules/loop-and-continuous-mode.md` § gotcha #7 (Bash heredoc apostrophe trap)
- `.claude/gdd/audits/audit-process.md` — where this template is used for audit agents
