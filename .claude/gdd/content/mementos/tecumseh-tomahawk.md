...
Tecumseh Leader Path memento. Tecumseh's personal weapon.

## Stats / numeric attributes

- **Path:** Leader — Tecumseh
- **Unlock level:** Leader Path L5
- **Effect:** Forest combat bonus
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Forest combat bonus `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: tecumseh
    level: 5
    description: "Forest combat bonus"
```
