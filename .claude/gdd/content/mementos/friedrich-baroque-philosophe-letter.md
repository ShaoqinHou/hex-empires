# Letter to Voltaire - Civ VII

**Slug:** `friedrich-baroque-philosophe-letter`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/friedrich-baroque.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Friedrich Baroque Leader Path memento. Philosopher-king correspondence.

## Stats / numeric attributes

- **Path:** Leader — Friedrich Baroque
- **Unlock level:** Leader Path L9
- **Effect:** Diplomatic Influence bonus
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Diplomatic Influence bonus `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    path: leader
    leader: friedrich-baroque
    level: 9
    description: "Diplomatic Influence bonus"
```
