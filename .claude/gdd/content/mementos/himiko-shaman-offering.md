# Offering Bowl - Civ VII

**Slug:** `himiko-shaman-offering`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/himiko-high-shaman.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Himiko High Shaman Leader Path memento. Ritual offerings to kami.

## Stats / numeric attributes

- **Path:** Leader — Himiko High Shaman
- **Unlock level:** Leader Path L5
- **Effect:** Happiness from Temples
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Happiness from Temples `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    level: 5
    description: "Happiness from Temples"
```
