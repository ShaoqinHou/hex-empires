# Traveler's Robe - Civ VII

**Slug:** `ibn-battuta-robe`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/ibn-battuta.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Ibn Battuta Leader Path memento. Scholar-traveler garments.

## Stats / numeric attributes

- **Path:** Leader — Ibn Battuta
- **Unlock level:** Leader Path L9
- **Effect:** Diplomatic bonus with foreign civs
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Diplomatic bonus with foreign civs `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: ibn-battuta
    level: 9
    description: "Diplomatic bonus with foreign civs"
```
