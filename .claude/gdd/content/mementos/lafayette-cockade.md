# Revolutionary Cockade - Civ VII

**Slug:** `lafayette-cockade`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/lafayette.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Lafayette Leader Path memento. French Revolution tricolor.

## Stats / numeric attributes

- **Path:** Leader — Lafayette
- **Unlock level:** Leader Path L2
- **Effect:** Culture during Celebrations
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Culture during Celebrations `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: lafayette
    level: 2
    description: "Culture during Celebrations"
```
