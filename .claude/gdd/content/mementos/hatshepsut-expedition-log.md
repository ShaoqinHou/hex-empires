# Punt Expedition Log - Civ VII

**Slug:** `hatshepsut-expedition-log`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/hatshepsut.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Hatshepsut Leader Path memento. Famous voyage to the Land of Punt.

## Stats / numeric attributes

- **Path:** Leader — Hatshepsut
- **Unlock level:** Leader Path L5
- **Effect:** +2 Gold per Trade Route
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. +2 Gold per Trade Route `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: hatshepsut
    level: 5
    description: "+2 Gold per Trade Route"
```
