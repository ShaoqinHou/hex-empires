# Kalinga Banner - Civ VII

**Slug:** `ashoka-conqueror-banner`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/ashoka-world-conqueror.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Ashoka World Conqueror Leader Path memento. Banner from the bloody Kalinga campaign.

## Stats / numeric attributes

- **Path:** Leader — Ashoka World Conqueror
- **Unlock level:** Leader Path L9
- **Effect:** Conquest yield bonus
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Conquest yield bonus `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    level: 9
    description: "Conquest yield bonus"
```
