...
- `.claude/gdd/content/leaders/pachacuti.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Pachacuti Leader Path memento. Inca knotted-cord record system.

## Stats / numeric attributes

- **Path:** Leader — Pachacuti
- **Unlock level:** Leader Path L5
- **Effect:** Science from Mountain adjacency
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Science from Mountain adjacency `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    leader: pachacuti
    level: 5
    description: "Science from Mountain adjacency"
```
