# Magatama Beads - Civ VII

**Slug:** `himiko-wa-beads`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/himiko-queen-of-wa.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Himiko Queen Of Wa Leader Path memento. Curved jade beads of Yayoi Japan.

## Stats / numeric attributes

- **Path:** Leader — Himiko Queen Of Wa
- **Unlock level:** Leader Path L5
- **Effect:** Culture bonus per Settlement
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Culture bonus per Settlement `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    level: 5
    description: "Culture bonus per Settlement"
```
