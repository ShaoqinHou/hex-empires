# Poor Richard's Almanac - Civ VII

**Slug:** `franklin-almanac`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/benjamin-franklin.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Benjamin Franklin Leader Path memento. Franklin's annual almanac of wisdom.

## Stats / numeric attributes

- **Path:** Leader — Benjamin Franklin
- **Unlock level:** Leader Path L2
- **Effect:** +1 Science per Library
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. +1 Science per Library `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
...
effects:
  - type: MEMENTO_BONUS
    ...
    level: 2
    description: "+1 Science per Library"
```
