# War Elephant Saddle - Civ VII

**Slug:** `trung-elephant-saddle`
**Category:** `memento`
**Age:** `ageless`
**Status:** `draft`
**Confidence:** `medium`
...
- `.claude/gdd/content/leaders/trung-tac.md` (parent leader fact card)
- `.claude/gdd/systems/mementos.md` + `.claude/gdd/systems/legends.md`

## Identity

Trung Tac Leader Path memento. Trưng sisters rode to battle on elephants.

## Stats / numeric attributes

- **Path:** Leader — Trung Tac
- **Unlock level:** Leader Path L2
- **Effect:** Cavalry bonus
- **Availability:** equippable on ANY leader once unlocked (cross-leader per VII design)

## Effect description

Pre-game equippable item. Effect persists the whole game. Cavalry bonus `[INFERRED]` exact numeric values may require in-game verification; memento effects in Civ VII combine flat bonuses, conditional triggers, and unit-class modifiers.
