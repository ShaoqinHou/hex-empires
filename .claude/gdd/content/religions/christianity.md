...
- https://civilization.fandom.com/wiki/Category:Religion_icons_(Civ7) — Fandom icon category (403; confirmed via web search snippet that Catholic, Protestant, and Orthodox icons are in vanilla set)

## Identity

- Historical period: Founded in 1st century CE Judea/Roman Palestine; spread westward through the Roman Empire and later globally via European Exploration-era missionaries — making it thematically well-matched to Civ VII's Exploration Age setting.
- Flavor: Christianity is represented in Civ VII as three separate cosmetic variants: Catholicism, Protestantism, and Eastern Orthodoxy — each with a distinct icon. All three are cosmetic; they share the same belief pool and have no mechanical differences from each other or from any other religion.
...
- Effect: Cosmetic only — no unique gameplay effect tied to the name "Christianity" (or its variants). All effects come from chosen beliefs.
...
### Cosmetic variants (all functionally identical)

- **Catholicism** — cross icon, Roman Catholic tradition
- **Protestantism** — alternate cross icon, Reformed tradition
- **Eastern Orthodoxy** — Orthodox cross icon, Byzantine/Eastern tradition

### Available Beliefs (shared pool — not Christianity-specific)
...
Christianity has three cosmetic sub-variants in the icon set (Catholicism, Protestantism, Orthodoxy), all sharing the same belief pool. The thematic fit with the Exploration Age is strong: European colonial-era Christianity spread via exactly the missionary mechanics Civ VII models. The `Evangelism` reliquary belief and `Stella Maris` enhancer belief (naval missionary movement) have obvious thematic resonance with historical Catholic missionary voyages. [LLM-KNOWLEDGE-ONLY] for historical period details.
