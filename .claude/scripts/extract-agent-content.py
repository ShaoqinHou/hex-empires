"""
Reusable utility: extract agent-written content blocks from a Claude Code
session transcript (JSONL) WITHOUT loading that content into the caller's
context window.
...
## Usage
    ...
    python .claude/scripts/extract-agent-content.py \
        [--transcript PATH] \
        [--output-root PATH] \
...
Defaults:
- transcript: latest .jsonl in ~/.claude/projects/<this-project>/
- output-root: .claude/gdd/content/
...
def default_transcript_path():
    ...
    patterns = [
        os.path.join(home, '.claude', 'projects', cwd, '*.jsonl'),
        os.path.join(home, '.claude', 'projects', '*hex-empires*', '*.jsonl'),
    ]
    for pat in patterns:
...
def main():
    ...
    ap.add_argument('--output-root', default='.claude/gdd/content',
