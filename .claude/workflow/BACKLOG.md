...
| WF-AUTO-11 | **NEW** — Orchestrator's final Write calls hit "permission prompt was declined" despite `--dangerously-skip-permissions`. Driver captured outputs to stdout log but couldn't persist `review-outcome-<sha>.md`, `last-review-summary.md`, or truncate queue. Root-cause: appears to be a Claude Code headless-spawn permission gap — `--dangerously-skip-permissions` may not cover all Write calls in long-running sessions. Workaround: human reconstructs files from log. Fix candidates: (a) investigate settings.json `permissions.allow` patterns for completeness, (b) spawn driver with `--permission-mode bypassPermissions` instead of (or alongside) `--dangerously-skip-permissions`, (c) structure the orchestrator to write files early/incrementally instead of at the end. | WF-AUTO-7 observation | pending |
| WF-AUTO-12 | **NEW** — Self-healing observed in WF-AUTO-7: the new orchestrator retroactively wrote `review-outcome-0a168ce.md` + `review-20deda2.md` (iter-2 on WF-AUTO-2b's Fixer commit) while processing bb4a89d — even though 0a168ce wasn't in its queue. This could be great (auto-heals missing outcome files on any drive) or concerning (unbounded retroactive work). Investigate: should the orchestrator only work on its assigned queue, or scan-and-fix historical state? | WF-AUTO-7 observation | pending |
