...
| WF-AUTO-14 | Workflow self-documentation refactor. | user request 2026-04-17 | **SUPERSEDED** — shipped 2026-04-17 with `principles.md`, `coordination.md`, `MANIFEST.md`, `inject-rules.sh`, `/audit-workflow` skill. Commits `202cd8e` + `fa1ae7c` (WF-OVERHAUL) then deleted all of those in favor of Claude Code built-ins (auto-loading `.claude/rules/*.md`, skill `paths:`, custom subagents). Surviving artifacts: `engine-patterns.md`, BACKLOG cleanup, continuation directive, commit-review PRINCIPLE-GAP step, skill-description triggers. |
