...
| WF-AUTO-7 | Re-validate AUTO_MODE with the new `git worktree add` flow. Next real BLOCK-producing commit should drop an isolated worktree, not touch main's checkout. | post-WF-AUTO-2b fix | pending |
| WF-AUTO-8 | Fixer contract: in WF-AUTO-2b the Fixer addressed all 3 findings (1 BLOCK + 2 WARNs) despite the "BLOCK-only" prompt. Decide: tighten prompt to strictly reject WARN edits, OR relax to "permit WARN fixes on the same lines as BLOCK". | WF-AUTO-2b observation | pending |
| WF-AUTO-9 | Orchestrator gap: the auto-drive on 0a168ce did NOT write `review-outcome-<sha>.md` or update `last-review-summary.md`. Driver exited before completing orchestrator steps 3-4. Investigate + add explicit finalization pass. | WF-AUTO-2b observation | pending |
| WF-AUTO-10 | Iter-2 Reviewer never ran on the Fixer's commit in WF-AUTO-2b (driver exited). Need to ensure the orchestrator loops back to step 2b after the Fixer succeeds. | WF-AUTO-2b observation | pending |
