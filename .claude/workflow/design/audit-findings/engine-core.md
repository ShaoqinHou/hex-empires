...
Per `.claude/rules/import-boundaries.md`, `state/` may import from `types/`, `hex/`, and `registry/` — never from `systems/`. These imports invert the layering: utilities that the web UI calls now indirectly depend on system implementation details.
