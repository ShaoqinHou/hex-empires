...
5. **Reviewer rules** added to `.claude/rules/` or inline grep patterns:
...
Estimate: 1 week for a developer to build the token sheet + typing + lint. Follow-up 2 weeks for the mechanical migration sweep across panels and HUDs. Migration is parallelizable (one agent per panel group in worktrees) per `.claude/skills/spawn-worktree/`.
