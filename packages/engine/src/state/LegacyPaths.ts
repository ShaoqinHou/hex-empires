...
/**
 * AA5.1: Count codices in the global state.codices map that belong to the player.
 * Codices are generated by researchSystem when a tech is researched with a library
 * or museum present. Falls back to 0 when state.codices is absent (pre-AA5.1 saves).
 */
function codicesCount(state: GameState, playerId: PlayerId): number {
  if (!state.codices) return 0;
  let n = 0;
  for (const codex of state.codices.values()) {
    if (codex.playerId === playerId) n += 1;
  }
  return n;
}
  ...
  milestones: [
    {
      ...
      tier: 1,
      // AA5.1: prefer codicesCount (state.codices) when available; fall back to
      // scienceLegacyScore (codexPlacements*10 + techs*1) proxy.
      // Threshold 1 codex or score 4 (e.g. 4 techs).
      description: 'Generate 1 Codex via tech research (fallback: science score 4)',
      check: (pid, s) => {
        if (s.codices) return codicesCount(s, pid) >= 1;
    ...
    },
    {
      ...
      tier: 2,
      // AA5.1: prefer codicesCount; fallback to scienceLegacyScore.
      description: 'Generate 3 Codices via tech research (fallback: science score 8)',
      check: (pid, s) => {
        if (s.codices) return codicesCount(s, pid) >= 3;
    ...
    },
    {
      ...
      tier: 3,
      // AA5.1: prefer codicesCount; fallback to scienceLegacyScore or library.
      description: 'Generate 6 Codices via tech research (fallback: science score 100 or Library in every city)',
      check: (pid, s) => {
        if (s.codices) return codicesCount(s, pid) >= 6;
