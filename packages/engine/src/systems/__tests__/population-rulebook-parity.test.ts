...
describe('§3.1 — Growth threshold formula (quadratic post-patch)', () => {
  it('R31: antiquity base threshold at pop=1 is 5 (X=0 → 5 + 0 + 0)', () => {
    expect(getGrowthThreshold(1, 'antiquity')).toBe(5);
  });

  it('R31: antiquity threshold at pop=5 is 5 + 20*4 + 4*16 = 149 (X=4)', () => {
    // X = pop - starting_pop (= pop - 1) = 4, X² = 16.
    // Flat 5 + Scalar 20·4 + Exponent 4·16 = 5 + 80 + 64 = 149.
    expect(getGrowthThreshold(5, 'antiquity')).toBe(149);
  });

  it('R31: exploration threshold at pop=5 is 30 + 50*4 + 5*16 = 310 (X=4)', () => {
    expect(getGrowthThreshold(5, 'exploration')).toBe(310);
  });

  it('R31: modern threshold at pop=5 is 60 + 60*4 + 6*16 = 396 (X=4)', () => {
    expect(getGrowthThreshold(5, 'modern')).toBe(396);
  });

  it('R31c: threshold difference between pop=5 and pop=2 is dominated by X² term', () => {
    // pop=2 (X=1): 5 + 20 + 4 = 29
    // pop=5 (X=4): 149. Difference 120 ≫ 29 — quadratic dominates.
    const t2 = getGrowthThreshold(2, 'antiquity');
    const t5 = getGrowthThreshold(5, 'antiquity');
    expect(t2).toBe(29);
    expect(t5).toBe(149);
    expect(t5 - t2).toBeGreaterThan(4 * t2); // quadratic blowup: 120 > 4·29 = 116
  });

  it('R31b: food accumulates turn-to-turn when surplus is positive and below threshold', () => {
    // pop=3, threshold=61, yield=8 food (3 grassland × 2 + city center 2) (II2 F-10), consumption=6 → surplus=2.
    // Two ticks should leave food near 4, far below threshold 61, and the city should not grow.
    const city = createTestCity({ population: 3, food: 0 });
    ...
    expect(c1.population).toBe(3);
    // Per docstring above: yield=8, consumption=6 → surplus=2. After tick1: food = 0 + 2 = 2.
    expect(c1.food).toBe(2);
  ...
  it('R31: pop=1 with enough food crosses threshold and grows to pop=2 on END_TURN', () => {
    // threshold = 5. Start at food = 4, surplus = 6 (yield 8 − consumption 2) (II2 F-10). 4+6=10 ≥ 5 → grow.
    ...
    // Leftover food = 10 − 5 = 5.
    expect(updated.food).toBe(5);
  });
});

describe('§3.2 — Growth rate modifiers', () => {
  it('R32a: Granary (growthRateBonus=0.1) lowers effective threshold vs no-building', () => {
    // At pop=2, antiquity base = 29. With 0.1 rate bonus → round(29 × 0.9) = 26.
    const city = createTestCity({ population: 2, buildings: ['granary'] });
    const state = createTestState({ cities: new Map([['c1', city]]) });
  ...
  it('R32d: deeply unhappy city (happiness ≤ -6) has reduced food growth rate', () => {
    ...
    const happyCity = createTestCity({ population: 2, food: 0, happiness: 10, territory });
    const unhappyCity = createTestCity({ population: 2, food: 0, happiness: -6, territory });
    ...
    // happyCity (happiness=10): base food=8, consumption=4 → surplus=4 → food=4.
    // unhappyCity (happiness=-6): Math.floor(8*0.88)=7 food, consumption=4, surplus=3 → food=3.
    expect(unhappyFood).toBeLessThan(happyFood);
    expect(unhappyFood).toBe(3);
  });
});

describe('§3.3 — Population allocation & specialists', () => {
  it('R33a: specialist food cost deducted in YieldCalculator (KK3.1 — F-02)', () => {
    // KK3.1: specialist food cost (2 per specialist) is now deducted in
    // calculateCityYields so the reported food/turn already reflects specialist
    // maintenance. growthSystem.foodConsumed no longer adds the specialist
    // component — it only uses population × 2.
    ...
    // 1 specialist: −2 food; 3 specialists: −6 food
    expect(baseYields.food - oneYields.food).toBe(2);
    expect(baseYields.food - threeYields.food).toBe(6);
  });
